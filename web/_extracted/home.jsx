// home.jsx — Editorial landing for wholesale showroom

function Home({ setScreen, setProductId }) {
  const featured = window.PRODUCTS.filter(p => p.featured);
  return (
    <main>
      {/* Hero — full-bleed with large editorial type */}
      <section style={{ position: "relative", height: "calc(100vh - 110px)", minHeight: 600 }}>
        <Photo tone="clay" ratio="auto" product={window.PRODUCTS.find(x=>x.id==="SOL-001")} view="front" caption="Look 01 · Solar · Atelier"
          style={{ position: "absolute", inset: 0, aspectRatio: "auto" }}/>
        <div style={{
          position: "absolute", inset: 0, padding: 48,
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 32 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "flex-end", minWidth: 320 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <Btn variant="accent" size="lg" onClick={() => setScreen("catalog")} icon={<I.Arrow/>}>Ver catálogo</Btn>
                <Btn variant="outline" size="lg" onClick={() => setScreen("lookbook")}
                  style={{ color: "white", borderColor: "white" }}>Lookbook</Btn>
              </div>
            </div>
          </div>
        </div>
        {/* Side index */}
        <div style={{
          position: "absolute", top: 48, right: 48, writingMode: "vertical-rl",
      ...
      {/* Strip of raw info — how it works */}
      ...
      {/* Featured products — editorial 3-up */}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 48 }}>
            <div>
              ...
              <h2 className="display" style={{ fontSize: 72 }}>
                Peças que <em>saem primeiro</em>.
              </h2>
            </div>
            <button onClick={() => setScreen("catalog")} style={{
              fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase",
              display: "inline-flex", alignItems: "center", gap: 8, borderBottom: "1px solid var(--brand-foreground)", paddingBottom: 4,
            }}>Todas as 115 peças <I.Arrow/></button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 4 }}>
            {featured.map(p => (
              <button key={p.id} onClick={() => { setProductId(p.id); setScreen("product"); }}
                style={{ textAlign: "left", display: "block" }}>
                <Photo tone={p.colors[0].tone} ratio="3/4" product={p}
                  caption={`${p.id}`}/>
      ...
      <section style={{ padding: "0 32px 96px" }}>
        <div style={{ maxWidth: 1440, margin: "0 auto" }}>
          ...
          <h2 className="display" style={{ fontSize: 72, marginBottom: 48 }}>
            Quatro linhas. <em>Um atelier.</em>
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
            {window.COLLECTIONS.map((c, i) => (
              <button key={c.id} onClick={() => setScreen("catalog")}
                ...
                <div style={{ marginTop: "auto" }}>
                  ...
                  <div style={{ fontSize: 13, color: "var(--brand-muted)", marginTop: 8 }}>
                    {c.season} · {c.pieces} peças
                  </div>
                </div>
              </button>
            ))}
      ...
      {/* Differentiator: Virtual try-on callout */}
      <section style={{ background: "var(--brand-foreground)", color: "white", padding: "96px 32px" }}>
        <div style={{ maxWidth: 1440, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          <div>
            ...
            <h2 className="display" style={{ fontSize: 84, lineHeight: 0.95 }}>
              Sua cliente <em>prova antes</em> de pedir.
            </h2>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
              <Btn variant="accent" size="lg" onClick={() => { setProductId("SOL-001"); setScreen("product"); }}>Abrir prova virtual</Btn>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
            ...
            <Photo tone="coral" ratio="3/4" product={window.PRODUCTS.find(x=>x.id==="SOL-001")} caption="Depois · SOL-001 · Coral · M"/>
          </div>
        </div>
      </section>

      {/* Pricing tiers */}
      <section style={{ padding: "96px 32px", background: "var(--brand-surface-2)" }}>
          ...
          <div style={{ background: "white", border: "1px solid var(--brand-border)" }}>
            {window.TIERS.map((t, i) => (
              <div key={i} style={{
                ...
                borderBottom: i < window.TIERS.length-1 ? "1px solid var(--brand-border)" : "none",
                fontSize: 15,
              }}>

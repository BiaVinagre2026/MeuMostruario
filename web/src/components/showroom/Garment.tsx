...
type GarmentType =
  | "biquini-triangle" | "biquini-cortina" | "biquini-hotpants"
  | "maio" | "fit-top" | "legging" | "short"
  | "saida" | "vestido" | "calca" | "chapeu" | "canga";
...
export function Garment({ type, color = "#C9A279", accent, size = 240, style = {} }: GarmentProps) {
  const fg = accent ?? "#1A1A1A";
  const shadow = "rgba(0,0,0,0.12)";
  const common = { fill: color, stroke: fg, strokeWidth: 0.8, strokeLinejoin: "round" as const, strokeLinecap: "round" as const };
  ...
  return (
    <svg viewBox="0 0 140 210" preserveAspectRatio="xMidYMid meet"
      ...
      {shapes[type] ?? shapes["biquini-triangle"]}
    </svg>
  );
}

const ID_MAP: Record<string, GarmentType> = {
...
export function garmentTypeFor(p: Pick<Product, "id" | "category">): GarmentType {
  if (ID_MAP[p.id]) return ID_MAP[p.id];
